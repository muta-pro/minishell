...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/30 17:24:45 by imutavdz          #+#    #+#             */
/*   Updated: 2025/12/30 18:33:26 by imutavdz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_if_exists(t_env **env, t_xp *xp)
{
	...
	while (curr_env)
	{
		if (ft_strcmp(curr_env->key, xp->key) == 0)
		{
			if (xp->eqptr)
			{
				free(curr_env->value);
...
void	parse_export_arg(t_ast_node *node, t_xp *xp)
{
	...
	xp->key_found = 0;
	if (xp->eqptr)
		xp->key = strndup(xp->curr, xp->eqptr - xp->curr);
	else
